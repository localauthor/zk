...
Out of the box, links are buttons made with the built-in =button.el=. This
means that links will work the same in any major mode --- fundamental-mode,
text-mode, outline-mode, markdown-mode, etc. (Note
that in Org-Mode, links in the default zk-link-format, ie, an ID in double-brackets, are
treated as Org-Links. Making them function properly as zk-links only requires
adding simple advice to the function =org-open-at-point=. See below for
details.)
...
*** Using Org-Mode /and/ double-bracketed links

In Org-Mode, when zk-links are in the default format =zk-link-format= ---,
that is IDs between double-brackets --- Org will want to treat them as
internal links, to headings or targets. This means they won't work as
zk-links in Org-Mode out of the box. 

You can get around this, however, by adding advice to the function
=org-open-at-point=, to instruct Org to try to follow a zk-link whenever
=org-open-at-point= fails:
...
=zk-link-format=, for example, to single brackets.

*** link-hint.el

In any mode, even Org-Mode, if [[https://github.com/noctuid/link-hint.el][link-hint.el]] is loaded, then
=link-hint-open-link= will find visible IDs. This is highly recommended!

**  Embark Integration

This package includes out-of-the-box support for [[https://github.com/oantolin/embark][Embark]], both on
links-at-point and in the minibuffer.
...
to follow links or to search for instances of the ID at point in all notes,
with =zk-search=.

Calling =embark-act= in the minibuffer makes available the functions in
=zk-file-map=. This offers a convenient way to open notes or insert links.

Additionally, adding =zk-search= to other Embark keymaps is a convenient way
to search all notes for any Embark target.
